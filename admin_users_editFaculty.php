<?php
  ...
  if(isset($_POST['edit_userFaculty']))
  {
    ...
    $queryUpdateUserFaculty = "UPDATE tblUsers SET last_name = upper('$lastName'), first_name = upper('$firstName'), middle_initial = upper('$middleInitial') WHERE user_id = '$userIDtoEdit'";

    $queryUpdateUserFacultyResult = mysqli_query($conn, $queryUpdateUserFaculty);

    //query for editing existing record in tblPassword, with matching records in tblUsers
    $queryUpdateUserFacultyPassword = "UPDATE tblPasswords SET password = '$password' WHERE user_id = '$userIDtoEdit'";
    $queryUpdateUserFacultyPasswordResult = mysqli_query($conn, $queryUpdateUserFacultyPassword);

    header('location: admin_users_faculty.php');

  }
    $userIDtoBeEdited = $_GET['user_id'];
    $queryUserFacultyToEdit = "SELECT * FROM tblUsers WHERE user_type LIKE('FACULTY') AND status LIKE('active') AND user_id = '$userIDtoBeEdited'";
    $queryUserFacultyToEditResult = mysqli_query($conn, $queryUserFacultyToEdit);

    $queryUserFacultyPasswordToEdit = "SELECT * FROM tblPasswords WHERE user_id = '$userIDtoBeEdited'";
    $queryUserFacultyPasswordToEditResult = mysqli_query($conn, $queryUserFacultyPasswordToEdit);

    $rowFacultyToEdit = mysqli_fetch_array($queryUserFacultyToEditResult);
    $rowFacultyPasswordToEdit = mysqli_fetch_array($queryUserFacultyPasswordToEditResult);
?>
<?php
  //start of session
  session_start();
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <br>
    <div class="col-lg-6">
      <form action="admin_users_editFaculty.php" method="post">
            <table class="table table-bordered" align="center">
              ...
              <tr>
                <td>User ID</td>
                <td>
                  <input type="text" name="edit_txtUserID" class="form-control" value="<?php echo $rowFacultyToEdit['user_id']; ?>" readonly="readonly">
                </td>
              </tr>
              <tr>
                <td>Password</td>
                <td>
                  <input type="text" name="edit_txtPassword" class="form-control" value="<?php echo $rowFacultyPasswordToEdit['password']; ?>" required="">
                </td>
              </tr>
              <tr>
                <td>Last Name</td>
                <td>
                  <input type="text" name="edit_txtLastName" class="form-control" required="" onkeypress="return alphaOnly(event);" style="text-transform: uppercase;" value="<?php echo $rowFacultyToEdit['last_name']; ?>">
                </td>
              </tr>
              <tr>
                <td>First Name</td>
                <td>
                  <input type="text" name="edit_txtFirstName" class="form-control" required="" onkeypress="return alphaOnly(event);" style="text-transform: uppercase;" value="<?php echo $rowFacultyToEdit['first_name']; ?>">
                </td>
              </tr>
              <tr>
                <td>Middle Initial</td>
                <td>
                  <input type="text" name="edit_txtMiddleInitial" class="form-control" onkeypress="return alphaOnly(event);" style="text-transform: uppercase;" value="<?php echo $rowFacultyToEdit['middle_initial']; ?>">
                </td>
              </tr>
              <tr>
                <td colspan="2">
                ...
                <td>
                  <input type="submit" name="edit_userFaculty" value="Edit Faculty Record" class="btn btn-default btn-flat">
                </td>
              </tr>
          </table>
        </form>
