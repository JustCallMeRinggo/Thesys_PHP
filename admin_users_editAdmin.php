<?php
  ...
  if(isset($_POST['edit_userAdmin']))
  {
    ...
    $queryUpdateUserAdmin = "UPDATE tblUsers SET last_name = upper('$lastName'), first_name = upper('$firstName'), middle_initial = upper('$middleInitial') WHERE user_id = '$userIDtoEdit'";
    $queryUpdateUserAdminResult = mysqli_query($conn, $queryUpdateUserAdmin);

    //query for editing existing record in tblPassword, with matching records in tblUsers
    $queryUpdateUserAdminPassword = "UPDATE tblPasswords SET password = '$password' WHERE user_id = '$userIDtoEdit'";
    $queryUpdateUserAdminPasswordResult = mysqli_query($conn, $queryUpdateUserAdminPassword);

    header('location: admin_users_admin.php');

  }
    $userIDtoBeEdited = $_GET['user_id'];
    $queryUserAdminToEdit = "SELECT * FROM tblUsers WHERE user_type LIKE('%admin%') AND status LIKE('active') AND user_id = '$userIDtoBeEdited'";
    $queryUserAdminToEditResult = mysqli_query($conn, $queryUserAdminToEdit);

    $queryUserAdminPasswordToEdit = "SELECT * FROM tblPasswords WHERE user_id = '$userIDtoBeEdited'";
    $queryUserAdminPasswordToEditResult = mysqli_query($conn, $queryUserAdminPasswordToEdit);

    $rowAdminToEdit = mysqli_fetch_array($queryUserAdminToEditResult);
    $rowAdminPasswordToEdit = mysqli_fetch_array($queryUserAdminPasswordToEditResult);
?>
<?php
  //start of session
  session_start();
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <br>
    <div class="col-lg-6">
      <form action="admin_users_editAdmin.php" method="post">
            <table class="table table-bordered" align="center">
              ...
              <tr>
                <td>User ID</td>
                <td>
                  <input type="text" name="edit_txtUserID" class="form-control" value="<?php echo $rowAdminToEdit['user_id']; ?>" readonly="readonly">
                </td>
              </tr>
              <tr>
                <td>Password</td>
                <td>
                  <input type="text" name="edit_txtPassword" class="form-control" value="<?php echo $rowAdminPasswordToEdit['password']; ?>" required="">
                </td>
              </tr>
              <tr>
                <td>Last Name</td>
                <td>
                  <input type="text" name="edit_txtLastName" class="form-control" required="" onkeypress="return alphaOnly(event);" style="text-transform: uppercase;" value="<?php echo $rowAdminToEdit['last_name']; ?>">
                </td>
              </tr>
              <tr>
                <td>First Name</td>
                <td>
                  <input type="text" name="edit_txtFirstName" class="form-control" required="" onkeypress="return alphaOnly(event);" style="text-transform: uppercase;" value="<?php echo $rowAdminToEdit['first_name']; ?>">
                </td>
              </tr>
              <tr>
                <td>Middle Initial</td>
                <td>
                  <input type="text" name="edit_txtMiddleInitial" class="form-control" onkeypress="return alphaOnly(event);" style="text-transform: uppercase;" value="<?php echo $rowAdminToEdit['middle_initial']; ?>">
                </td>
              </tr>
              <tr>
                <td colspan="2">
                ...
                <td>
                  <input type="submit" name="edit_userAdmin" value="Edit Admin Record" class="btn btn-default btn-flat">
                </td>
              </tr>
          </table>
        </form>
